 ...
 */
class Gradient3DGenerator : public ProgramGenerator {
public:

  Gradient3DGenerator();

  virtual ~Gradient3DGenerator();
...
Gradient3DGenerator::Gradient3DGenerator() {
}

Gradient3DGenerator::~Gradient3DGenerator() {
}

std::string Gradient3DGenerator::generate(GeneratorParams& params) {
...
void Gradient3DGenerator::generateHeader(std::ostream& stream,
                                       const GeneratorParams& params) {
...
void Gradient3DGenerator::generateFooter(std::ostream& stream) {
  stream << "}\n\n";
}

void Gradient3DGenerator::generateLocals(std::ostream& stream,
                                       const GeneratorParams& params) {
...
void Gradient3DGenerator::generateCompute(std::ostream& stream,
                                        const GeneratorParams& params) {
  ...
  for(int32_t i = 0; i < params.elementsPerThread; ++i) {
    ...
    stream << "    val6 = *(inputPtr+(" << params.paddedSize << "*" << i
           << ")+(" << params.paddedSize*params.paddedSize << "*1));";
    stream << "    " << params.dataType
           << " result = val1 + rsqrt(0.0001f + SQR(val1-val0) + SQR(val1-val2) "
           << "+ SQR(val1-val3) + SQR(val1-val4) + SQR(val1-val5) + SQR(val1-val6));\n";
...
#define ARRAY_REF(A, i, j, k)                                         \
  ...
  double gflops   = (double)params.realSize * (double)params.realSize
    ...
    * 20.0 * (double)params.timeSteps / elapsed / 1e9;
  ...
  gflops = (double)params.blockSizeX * (double)params.blockSizeY
    ...
    * (double)params.elementsPerThread * 20.0 * (double)params.timeSteps
    / elapsed / 1e9;

  printValue("phase2_global_loads", 7.0);
  printValue("phase2_shared_loads", 0.0);
  printValue("compute_per_point", 20.0);
  printValue("phase3_shared_loads", 7.0);
