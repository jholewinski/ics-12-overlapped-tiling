    ...
    for bsx in block_x:
      for bsy in block_y:
        for bsz in block_z:
          ...
          try:
            block_size_z = float(program_data['Block Size Z'])
          except:
            block_size_z = 1.0

          num_warps_per_block = block_size_x * block_size_y * block_size_z / 32.0
          ...
          try:
            num_blocks_z = float(program_data['Num Blocks Z'])
          except:
            num_blocks_z = 1.0
          ...
          total_blocks_per_sm = math.ceil(num_blocks_x * num_blocks_y * num_blocks_z / num_sm)
          ...
          print('%d,%d,%d,%d,%d,%d,%f,%E,%f,%f,%f,' % (bsx, bsy, bsz, tts, elems, phase_limit, elapsed_time, variance, sim_time, actual_gflops, occupancy))
          sys.stdout.flush()
          log.flush()
